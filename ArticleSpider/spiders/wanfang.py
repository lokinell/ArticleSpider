...
class WanfangSpider(scrapy.Spider):
    ...
    start_urls = [
        'http://s.wanfangdata.com.cn/Paper.aspx?q=心血管&f=top&p=1'
    ]
    ...
    def parse(self, response):
        ...
        count = sel.xpath(
            '/html/body/div[@class="content content-search clear"]/div[@class="right"]/div[@class="record-item-list"]/p[@class="pager"]/span[@class="page_link"]/text()').extract()
    ...
    def parse_user_detail(self, response):
        sel = Selector(response)
        url_s = sel.xpath(
            '/html/body/div[@class="content content-search clear"]/div[@class="right"]/div[@class="record-item-list"]/div[@class="record-item"]/div[@class="left-record"]/div[@class="record-title"]/a[@class="title"]/@href').extract()
        for i in url_s:
    ...
    def parse_page(self, response):
        ...
        body = sel.xpath(
            "/html/body/div[@class='fixed-width-wrap fixed-width-wrap-feild']/div[@class='fixed-width baseinfo-feild']")
        item = WanfangItem()
        C_title = head.xpath("//h1/text()").extract()
        E_title = head.xpath("//h2/text()").extract()
        abstract = head.xpath(
            "//div[@class='baseinfo-feild abstract']/div[@class='row clear zh']/div[@class='text']/text()").extract()
